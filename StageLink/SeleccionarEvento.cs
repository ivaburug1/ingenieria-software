...
namespace StageLink
{
    public partial class SeleccionarEvento : Form, IObserver_391IAU
    {
        ...
        private void CBSeleccionarEvento_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            CBSeleccionarFecha.SelectedIndex = -1;
        }
        ...
        private void CBSeleccionarFecha_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            BLL_Evento bll = new BLL_Evento();
            var (estadio, direccion) = bll.ObtenerEstadioYDireccion(artista, fecha);
        ...
        private void BTNCobrarBoleto_Click(object sender, EventArgs e)
        {
            ...
            BLLSector bllSec = new BLLSector();
            int disponibles = bllSec.ObtenerDisponibles(codigoEvento, sectorCodigo);
            if (cantidad > disponibles)
            {
                MessageBox.Show(
                    $"Solo quedan {disponibles} entradas disponibles para ese sector.",
                    "Stock insuficiente",
        ...
        private void CBSeleccionarSector_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                BLL_Evento bllEvento = new BLL_Evento();
                int codigoEvento = bllEvento.ObtenerCodigoEvento(
                    CBSeleccionarEvento.SelectedItem.ToString(),
                    (DateTime)CBSeleccionarFecha.SelectedItem);

                BLLSector bllSector = new BLLSector();
                int disponibles = bllSector.ObtenerDisponibles(codigoEvento, sector.Codigo);

                LBLNotasSector.Text = $"Sector {sector.Nombre}: precio {sector.Precio:C}, disponibles: {disponibles}.";
            }
        }
